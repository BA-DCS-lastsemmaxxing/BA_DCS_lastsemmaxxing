...
export function DocumentModal({ isOpen, onOpenChange, document }: DocumentModalProps) {
  ...
  const handleFeedbackSubmit = async () => {
    ...
    // http://127.0.0.1:5001/
    try {
      const response = await fetch(`https://kay8ehgv4g.execute-api.ap-southeast-1.amazonaws.com/Test/send_feedback/send_feedback/?document_id=${encodeURIComponent(String(document.id))}`, {
        method: 'POST',
        headers: {
