...
# Create /documents/{documentId} for DELETE operation
resource "aws_api_gateway_resource" "document_by_id_resource" {
  rest_api_id = aws_api_gateway_rest_api.lsm-fyp-api.id
  parent_id   = aws_api_gateway_resource.documents_resource.id
  path_part   = "{documentId}"
}

# DELETE method for /documents/{documentId}
resource "aws_api_gateway_method" "document_delete_method" {
  rest_api_id   = aws_api_gateway_rest_api.lsm-fyp-api.id
  resource_id   = aws_api_gateway_resource.document_by_id_resource.id
  http_method   = "DELETE"
  authorization = "COGNITO_USER_POOLS"
  authorizer_id = aws_api_gateway_authorizer.lsm-fyp-authorizer.id
}

# Integration for DELETE method (Lambda proxy)
resource "aws_api_gateway_integration" "document_delete_integration" {
  rest_api_id = aws_api_gateway_rest_api.lsm-fyp-api.id
  resource_id = aws_api_gateway_resource.document_by_id_resource.id
  http_method = aws_api_gateway_method.document_delete_method.http_method
...
resource "aws_lambda_permission" "document_delete_lambda_permission" {
...
# CORS for DELETE method (OPTIONS)
resource "aws_api_gateway_method" "document_delete_method_options" {
  rest_api_id   = aws_api_gateway_rest_api.lsm-fyp-api.id
  resource_id   = aws_api_gateway_resource.document_by_id_resource.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "document_delete_options_integration" {
  rest_api_id = aws_api_gateway_rest_api.lsm-fyp-api.id
  resource_id = aws_api_gateway_resource.document_by_id_resource.id
  http_method = aws_api_gateway_method.document_delete_method_options.http_method
  type        = "MOCK"
}

resource "aws_api_gateway_method_response" "document_delete_method_options_response" {
  rest_api_id = aws_api_gateway_rest_api.lsm-fyp-api.id
  resource_id = aws_api_gateway_resource.document_by_id_resource.id
  http_method = aws_api_gateway_method.document_delete_method_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "document_delete_method_options_integration_response" {
  rest_api_id = aws_api_gateway_rest_api.lsm-fyp-api.id
  resource_id = aws_api_gateway_resource.document_by_id_resource.id
  http_method = aws_api_gateway_method.document_delete_method_options.http_method
  status_code = "200"
